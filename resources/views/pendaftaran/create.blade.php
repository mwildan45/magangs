...
<h3 class="job-title text-center" style="font-weight: bold;font-size: 22px;">{{(Auth::user()->level == 'siswa') ? 'Isi Data Diri Kamu dan Mulailah Mencari !' : 'Isi Formulir Pendaftaran Siswa'}}</h3>
<p style="text-align: center;margin-bottom: 40px !important;">{{(Auth::user()->level == 'siswa') ? 'Isi dahulu data diri kamu agar DU/DI yang kamu pilih bisa melihat pengajuan kamu' : 'Section ini berguna untuk mengisi data formulir penerimaan siswa. Anda bisa langsung mempostingnya atau ditunda sampai waktu yang anda tentukan.'}}</p>
<form method="POST" action="{{ route('data.store') }}" enctype="multipart/form-data" class="form-ad" id="dudi_acc">
@if(Auth::user()->level == 'siswa')
	@include('pendaftaran._form_siswa')
@else
	@include('pendaftaran._form_dudi')
@endif
</form>
</div>
</div>
